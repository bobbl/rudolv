...
#define UART_RECV_CHAR          0x100
#define UART_SEND_CHAR          0x200
...
int uart_nonblocking_receive()
{
    unsigned long ch = read_csr(UART_CSR);
    if (ch & UART_RECV_BUF_EMPTY) return -1;
    write_csr(UART_CSR, UART_RECV_CHAR);
    return ch & 0xFF;
}
...
void uart_send(unsigned ch)
{
    while (read_csr(UART_CSR) & UART_SEND_BUF_FULL) {}
    write_csr(UART_CSR, UART_SEND_CHAR | ch);
}
